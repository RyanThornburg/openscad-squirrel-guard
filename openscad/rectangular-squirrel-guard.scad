...
function hole_x(idx, count, r) =
    let(gap = (panel_width - 2*count*r) / (count + 1))
    r + gap*(idx+1) + 2*r*idx;
...
_eff_front_clear = front_row_enabled
    ? max(front_hole_clearance, front_spikes_enabled ? front_spike_clearance + front_spike_base : front_hole_clearance)
    : 0;
_eff_back_clear = back_row_enabled
    ? max(back_hole_clearance, back_spikes_enabled ? back_spike_clearance + back_spike_base : back_hole_clearance)
    : 0;
...
function slot_overlaps_front(sx, sy) =
    ...
    let(thresh2 = let(t = front_hole_radius + _eff_front_clear + max(sw,sl)/2) t*t)
    len([for (i = [0:front_hole_count-1])
        if (dist2sq(sx, sy, front_hole_x(i), front_row_y) < thresh2) 1]) > 0;

function slot_overlaps_back(sx, sy) =
    !back_row_enabled ? false :
    let(thresh2 = let(t = back_hole_radius + _eff_back_clear + max(sw,sl)/2) t*t)
    len([for (i = [0:back_hole_count-1])
        if (dist2sq(sx, sy, back_hole_x(i), back_row_y) < thresh2) 1]) > 0;

function slot_overlaps_any_hole(sx, sy) =
    slot_overlaps_front(sx, sy) || slot_overlaps_back(sx, sy);

function slot_fits_panel(sx, sy) =
    (sx - sw/2 >= border) && (sx + sw/2 <= panel_width  - border) &&
    (sy - sl/2 >= border) && (sy + sl/2 <= panel_length - border);

// ─── Spike helpers 

function spike_in_rect(sx, sy, b) =
    (sx - b/2 >= 0) && (sx + b/2 <= panel_width) &&
    (sy - b/2 >= 0) && (sy + b/2 <= panel_length);

// Spike must not have its centre inside any plant hole
function spike_hits_front_hole(sx, sy, b) =
    !front_row_enabled ? false :
    let(thresh2 = (front_hole_radius + b/2) * (front_hole_radius + b/2))
    len([for (i = [0:front_hole_count-1])
        if (dist2sq(sx, sy, front_hole_x(i), front_row_y) < thresh2) 1]) > 0;

function spike_hits_back_hole(sx, sy, b) =
    !back_row_enabled ? false :
    let(thresh2 = (back_hole_radius + b/2) * (back_hole_radius + b/2))
    len([for (i = [0:back_hole_count-1])
        if (dist2sq(sx, sy, back_hole_x(i), back_row_y) < thresh2) 1]) > 0;
...
function spike_over_slot(spx, spy, b) =
    ...
    let(_shx = sw/2, _shy = sl/2, _sr = min(sw,sl)/2,
        ...
        mc  = max(0, bc-sc), xc = min(SLOT_COLS-1, bc+sc),
        mr  = max(0, br-sr), xr = min(SLOT_ROWS-1, br+sr))
    len([for (col=[mc:xc], row=[mr:xr])
        let(scx = SLOT_X0 + col*pitch_x, scy = SLOT_Y0 + row*pitch_y)
        if (slot_fits_panel(scx,scy) && !slot_overlaps_any_hole(scx,scy) &&
            dist_to_rrect(spx,spy,scx,scy,_shx,_shy,_sr) < b/2) 1]) > 0;

// Spike self-collision check
function spike_collides(sx, sy, b, placed) =
    len([for (p = placed)
        if (dist2sq(sx,sy,p[0],p[1]) < (b+p[2])*(b+p[2])/4) 1]) > 0;

function spike_placeable(sx, sy, b, placed) =
    spike_in_rect(sx, sy, b) &&
    !spike_hits_any_hole(sx, sy, b) &&
    !spike_collides(sx, sy, b, placed) &&
    (spike_priority || !spike_over_slot(sx, sy, b));

// Collect spike candidates
function _collect(pts, i, acc, b, t) =
    i >= len(pts) ? acc :
    let(sx = pts[i][0], sy = pts[i][1],
...
function _hole_ring(hx, hy, hole_r, clearance, base, count, t) =
    let(ring_r = hole_r + clearance + base/2 + gtol,
        n      = max(1, count),
        a_step = 360/n,
...
function _border_spike_acc(hole_acc) =
    ...
    let(nl    = max(1, border_spikes_on_length_edges),
        ...
        bot   = [for (i=[0:nl-1])
                    [x0 + (nl>1 ? (x1-x0)/(nl-1)*i   : (x1-x0)/2), m]],
        right = [for (i=[0:nw-1])
                    [panel_width-m, y0 + (nw>1 ? (y1-y0)/(nw-1)*i : (y1-y0)/2)]],
        top   = [for (i=[0:nl-1])
                    [x1 - (nl>1 ? (x1-x0)/(nl-1)*i   : (x1-x0)/2), panel_length-m]],
        left  = [for (i=[0:nw-1])
                    [m, y1 - (nw>1 ? (y1-y0)/(nw-1)*i : (y1-y0)/2)]],
...
function slot_hits_spike(sx, sy) =
    let(_shx = sw/2, _shy = sl/2, _sr = min(sw,sl)/2)
    len([for (p = ALL_SPIKES)
        if (dist_to_rrect(p[0], p[1], sx, sy, _shx, _shy, _sr) < p[2]/2) 1]) > 0;
...
// Precomputed once at parse time — avoids re-evaluating slot conditions
// on every segment intersection call during preview/render.
VALID_SLOTS = [for (col=[0:SLOT_COLS-1], row=[0:SLOT_ROWS-1])
    let(sx = SLOT_X0 + col*pitch_x,
        sy = SLOT_Y0 + row*pitch_y)
    if (slot_fits_panel(sx,sy) &&
        !slot_overlaps_any_hole(sx,sy) &&
        (!spike_priority || !slot_hits_spike(sx,sy)))
    [sx, sy]];

module slot_grid() {
    for (slot = VALID_SLOTS)
        rounded_slot(slot[0], slot[1]);
}
